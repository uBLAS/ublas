...
namespace boost { namespace numeric { 
    ...
    */
     ...
     * This matrix is directly based on a predefined C-style arry of data, thus providing the fastest
     * implementation possible. The constraint is that dimensions of the matrix must be specified at 
     * the instanciation or the type specification. This is same as the c_matrix except that it guarantees Alignment 
     *
     * For instance, \code typedef c_matrix_aligned<double,4,4> my_4by4_matrix \endcode 
     * defines a 4 by 4 double-precision matrix.  You can also instantiate it directly with 
     * \code c_matrix_aligned<int,8,5> my_fast_matrix \endcode. This will make a 8 by 5 integer matrix. The 
     * price to pay for this speed is that you cannot resize it to a size larger than the one defined 
     * in the template parameters. In the previous example, a size of 4 by 5 or 3 by 2 is acceptable, 
     * but a new size of 9 by 5 or even 10 by 10 will raise a bad_size() exception.
     *
     ...
     * \tparam M the default maximum number of columns
     */
    template<class T, std::size_t N, std::size_t M, std::size_t Alignment>
    class c_matrix_aligned:
        public matrix_container<c_matrix_aligned<T, N, M, Alignment> > {

        typedef c_matrix_aligned<T, N, M, Alignment> self_type;
    public:
#ifdef BOOST_UBLAS_ENABLE_PROXY_SHORTCUTS
        using matrix_container<self_type>::operator ();
#endif
        ...
        typedef const matrix_reference<const self_type> const_closure_type;
        typedef matrix_reference<self_type> closure_type;
        typedef c_vector<T, N * M> vector_temporary_type;     // vector able to store all elements of c_matrix_aligned
        typedef self_type matrix_temporary_type;
        typedef dense_tag storage_category;
        // This could be better for performance,
        // typedef typename unknown_orientation_tag orientation_category;
        ...
        BOOST_UBLAS_INLINE
        c_matrix_aligned (size_type size1, size_type size2):
            size1_ (size1), size2_ (size2) /* , data_ () */ {
            if (size1_ > N || size2_ > M)
                bad_size ().raise ();
        }
        BOOST_UBLAS_INLINE
        c_matrix_aligned (const c_matrix_aligned &m):
            size1_ (m.size1_), size2_ (m.size2_) /* , data_ () */ {
            if (size1_ > N || size2_ > M)
                bad_size ().raise ();
            assign(m);
        ...
        template<class AE>
        BOOST_UBLAS_INLINE
        c_matrix_aligned (const matrix_expression<AE> &ae):
            size1_ (ae ().size1 ()), size2_ (ae ().size2 ()) /* , data_ () */ {
            if (size1_ > N || size2_ > M)
                bad_size ().raise ();
            matrix_assign<scalar_assign> (*this, ae);
        ...
        BOOST_UBLAS_INLINE
        const_pointer data () const {
            return reinterpret_cast<const_pointer> (pool_.data_);
        }
        BOOST_UBLAS_INLINE
        pointer data () {
            return reinterpret_cast<pointer> (pool_.data_);
        }
        ...
        void resize (size_type size1, size_type size2, bool preserve = true) {
            ...
            if (preserve) {
                ...
                for (size_type i = 0; i != size1_min; ++i) {    // indexing copy over major
                    for (size_type j = 0; j != size2_min; ++j) {
                        temporary.pool_.data_[i][j] = pool_.data_[i][j];
                    }
                }
                assign_temporary (temporary);
            }
        ...
        void clear () {
            for (size_type i = 0; i < size1_; ++ i)
                std::fill (pool_.data_ [i], pool_.data_ [i] + size2_, value_type/*zero*/());
        }
        ...
        /*! @note "pass by value" the key idea to enable move semantics */
        BOOST_UBLAS_INLINE
        c_matrix_aligned &operator = (c_matrix_aligned m) {
            assign_temporary(m);
            return *this;
        }
#else
        BOOST_UBLAS_INLINE
        c_matrix_aligned &operator = (const c_matrix_aligned &m) {
            size1_ = m.size1_;
            size2_ = m.size2_;
            for (size_type i = 0; i < m.size1_; ++ i)
                std::copy (m.pool_.data_ [i], m.pool_.data_ [i] + m.size2_, pool_.data_ [i]);
            return *this;
        }
#endif
        template<class C>          // Container assignment without temporary
        BOOST_UBLAS_INLINE
        c_matrix_aligned &operator = (const matrix_container<C> &m) {
            resize (m ().size1 (), m ().size2 (), false);
            assign (m);
            return *this;
        }
        BOOST_UBLAS_INLINE
        c_matrix_aligned &assign_temporary (c_matrix_aligned &m) {
            swap (m);
            return *this;
        }
        template<class AE>
        BOOST_UBLAS_INLINE
        c_matrix_aligned &operator = (const matrix_expression<AE> &ae) { 
            self_type temporary (ae);
            return assign_temporary (temporary);
        }
        template<class AE>
        BOOST_UBLAS_INLINE
        c_matrix_aligned &assign (const matrix_expression<AE> &ae) { 
            matrix_assign<scalar_assign> (*this, ae); 
            return *this;
        }
        template<class AE>
        ...
        template<class AE>
        BOOST_UBLAS_INLINE
        c_matrix_aligned &plus_assign (const matrix_expression<AE> &ae) { 
            matrix_assign<scalar_plus_assign> (*this, ae); 
            return *this;
        }
        template<class AE>
        ...
        template<class AE>
        BOOST_UBLAS_INLINE
        c_matrix_aligned &minus_assign (const matrix_expression<AE> &ae) { 
            matrix_assign<scalar_minus_assign> (*this, ae); 
            return *this;
        }
        template<class AT>
        BOOST_UBLAS_INLINE
        c_matrix_aligned& operator *= (const AT &at) {
            matrix_assign_scalar<scalar_multiplies_assign> (*this, at);
            return *this;
        }
        template<class AT>
        ...
        void swap (c_matrix_aligned &m) {
            if (this != &m) {
                ...
                std::swap (size2_, m.size2_);
                for (size_type i = 0; i < size1_; ++ i)
                    std::swap_ranges (pool_.data_ [i], pool_.data_ [i] + size2_, m.pool_.data_ [i]);
            }
        }
        BOOST_UBLAS_INLINE
        friend void swap (c_matrix_aligned &m1, c_matrix_aligned &m2) {
            m1.swap (m2);
        }
    ...
    public:
        ...
        const_iterator1 find1 (int rank, size_type i, size_type j) const {
            ...
            return const_iterator1 (*this, i, j);
#else
            return const_iterator1 (*this, &pool_.data_ [i] [j]);
#endif
        }
        BOOST_UBLAS_INLINE
        iterator1 find1 (int rank, size_type i, size_type j) {
            ...
            return iterator1 (*this, i, j);
#else
            return iterator1 (*this, &pool_.data_ [i] [j]);
#endif
        }
        BOOST_UBLAS_INLINE
        const_iterator2 find2 (int rank, size_type i, size_type j) const {
            ...
            return const_iterator2 (*this, i, j);
#else
            return const_iterator2 (*this, &pool_.data_ [i] [j]);
#endif
        }
        BOOST_UBLAS_INLINE
        iterator2 find2 (int rank, size_type i, size_type j) {
            ...
            return iterator2 (*this, i, j);
#else
            return iterator2 (*this, &pool_.data_ [i] [j]);
#endif
        }
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
        class const_iterator1:
            public container_const_reference<c_matrix_aligned>,
            public random_access_iterator_base<dense_random_access_iterator_tag,
                                               const_iterator1, value_type> {
        public:
            typedef typename c_matrix_aligned::difference_type difference_type;
            typedef typename c_matrix_aligned::value_type value_type;
            typedef typename c_matrix_aligned::const_reference reference;
            typedef typename c_matrix_aligned::const_pointer pointer;
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
        class iterator1:
            public container_reference<c_matrix_aligned>,
            public random_access_iterator_base<dense_random_access_iterator_tag,
                                               iterator1, value_type> {
        public:
            ...
            typedef typename c_matrix_aligned::difference_type difference_type;
            typedef typename c_matrix_aligned::value_type value_type;
            typedef typename c_matrix_aligned::reference reference;
            typedef typename c_matrix_aligned::pointer pointer;
        ...
        public:
            ...
            typedef typename c_matrix_aligned::value_type value_type;
            typedef typename c_matrix_aligned::const_reference reference;
            typedef typename c_matrix_aligned::const_reference pointer;
        ...
        void serialize(Archive & ar, const unsigned int /* file_version */){
            ...
            // could probably use make_array( &(data[0][0]), N*M ) 
            ar & serialization::make_array(pool_.data_, N);
        }
