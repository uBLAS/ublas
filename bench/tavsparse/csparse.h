...
namespace tav {
    ...
    class SparseMatrix {
        /* structure consists of following
         ...
         - ind[i][j] gives for the row i, the column number of j-th nonzero element
         - elem[i][j] gives the numerical value of corresponding to ind[i][j] element
         */
        /*  protected:
         cnumber *elem;
         int    *nz;
    ...
    public:
        ...
        cnumber zeroelement;  //this controls zero elements, allows to use reference
        ...
        SparseMatrix(SparseMatrix<rnumber> &); //constructor
    ...
    {
        if (Abs(mat.zeroelement)>precision) mat.Sync();
        zeroelement=0.0;
        zeroi=n;
        zeroj=n;
        precision=1E-10;
    ...
    {
        if (Abs(zeroelement)>precision) Sync();
        if ((ii>=n)||(jj>=n)) FatalError ("SparseMatrix::operator(int,int)\nBad arguments.\n");
        uint_nbasis i,j;
    ...
    {
        if (Abs(zeroelement)>precision) Sync();
        if ((ii>=n)||(jj>=n)||(jj<0)||(ii<0)) FatalError ("SparseMatrix::operator(int,int)\nBad arguments.\n");
        //	uint_nbasis i,j;
        //        if (ii<=jj) {i=ii;j=jj;}  else {i=jj; j=ii;};
        if ((type)&&(ii>jj)) tav::Swap(ii,jj);
        for (unsigned q=0;q<nz[ii];q++)
        {
            if (ind[ii][q]==jj) return elem[ii][q]; }
        /*this matrix elemet is not in the list*/
    ...
    void  SparseMatrix<cnumber>::TimesMatrix(cnumber **yy, cnumber **xx, unsigned m) const {
        ...
        //cerr<<"Multiplication with shift : "<<shift<<endl;
        if (Abs(zeroelement)>precision) FatalError("no sync done before SparseMatrix::TimesMatrix");
...
//#pragma omp parallel for schedule(dynamic, 1)
            ...
            for (uint_nbasis i=0;i<n;i++) { //go over all elements
                for (unsigned k=0;k<nz[i];k++) {
                    ...
                    if (type)
                    {
                        if(i!=ind[i][k])
                        {
                            yy[im][ind[i][k]]+=elem[i][k]*xx[im][i];
                        }
                    }
    ...
    uint_nbasis SparseMatrix<cnumber>::Sync(void) {
        ...
        /*
        if (Abs(zeroelement)<precision) return 0; //should not call sync
        
        
        uint_nbasis tqr[nz[zeroi]+1];
        cnumber th[nz[zeroi]+1];
        uint_nbasis i=0;
        ...
        delete[] elem[zeroi];
        delete[] ind[zeroi];
        elem[zeroi]=new cnumber[nz[zeroi]+1];
        ind[zeroi]=new uint_nbasis[nz[zeroi]+1];
        nz[zeroi]++;
        for (uint_nbasis k=0;k<nz[zeroi];++k)
        {
            elem[zeroi][k]=th[k];
            ind[zeroi][k]=tqr[k];
        }
        //    cout<<"Here with: "<<zeroi<<" "<<nz[zeroi]+1<<endl;getchar();
        //    delete []tqr;
        //    delete []th;
    ...
    {if (Abs(a.zeroelement)>a.precision) a.Sync();
        if ((a.n!=n)) FatalError("'SparseMatrix=SparseMatrix':\nmatrices has diffirent dimentions");
        zeroelement=0.0;
        zeroi=n;
        zeroj=n;
    ...
    /*
     ...
     type =1; 
     for (int j=0;j<=i;j++){ 
     if (Abs(a(i,j))>precision) nz[i]++;
     if (Abs(a(i,j)-a(j,i))>precision) ErrorMsg("Error in 'SparseMatrix=Double_Matrix':\nDouble matrix is not symmetric");
     }
     if (ind[i]) delete []ind[i];
     if (elem[i]) delete []elem[i]; 
     ind[i]=new int [nz[i]];
