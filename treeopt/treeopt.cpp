...
public:
    ...
    Dense_Matrix(const std::string& name, int n = 1) : m_name(name) {
    ...
    size_t size1() const { return data.size(); }
    
    size_t size2() const { return data[0].size(); }
...
public:
    ...
    static NMatXpr build(const MatXpr& mxpr) { return Tree_Optimizer<C>::build(mxpr.matrixl) + mxpr.matrixr; }
};
...
int main(){
	...
	Dense_Matrix<2, 2> a("a", 3), b("b", 3), c("c", 3), d("d", 3);
  
    auto xpr = a + b * c + d + d * a;
