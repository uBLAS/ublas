...
namespace boost { namespace numeric { 
...
#endif
        typedef const matrix& nested_type;
    ...
    // Importing the operators from matrix_expression.
    // This is necessary for proper operation of the tree optimizer.
    ...
    /**
        A dynamic size row vector of values.
    */
    template <class T, class L, class A>
    using dynamicrowvector = matrix<T, L>;
        
    /**
        A dynamic size row vector of values.
    */
    template <class T, class L, class A>
    using dynamiccolvector = matrix<T, L>;
    ...
    public:
        ...
        typedef const matrix_reference<const self_type> const_closure_type;
        typedef matrix_reference<self_type> closure_type;
        typedef const fixed_matrix& nested_type;
        typedef vector<T, A> vector_temporary_type;
        typedef self_type matrix_temporary_type;
        typedef dense_tag storage_category;
        // This could be better for performance,
       ...
       */
        fixed_matrix (const value_type &init):
            matrix_container<self_type> (), data_ (init) { }
       ...
       */
        ...
        BOOST_UBLAS_INLINE
        BOOST_CONSTEXPR size_type rows () const {
            return M;
        }
       ...
       */
        ...
        BOOST_UBLAS_INLINE
        BOOST_CONSTEXPR size_type cols () const {
            return N;
        }
            
        /**
        * Return the size of a fixed vector.
        * If M == 1 (row vector) then this returns N.
        * If N == 1 (column vector) then this returns M.
        */
        BOOST_UBLAS_INLINE BOOST_CONSTEXPR
        typename boost::enable_if< mpl::or_<
        mpl::equal< mpl::size_t<M>, mpl::size_t<1> >,
        mpl::equal< mpl::size_t<N>, mpl::size_t<1> > >, size_type>::type size () const {
            return M == 1 ? N : M;
        }
    ...
    /** Access a vector element. Here we return a const reference
    * \param i the first coordinate of the element. By default it's the row
    * \param j the second coordinate of the element. By default it's the column
    * \return a const reference to the element
    */
        ...
        typename boost::enable_if< mpl::or_<
        mpl::equal< mpl::size_t<M>, mpl::size_t<1> >,
        mpl::equal< mpl::size_t<N>, mpl::size_t<1> > >,
        const_reference >::type operator () (size_type i) const {
            return M == 1 ? data () [layout_type::element (size_type(0), M, i, N)] :
            data () [layout_type::element (i, M, size_type(0), N)];
        }
            
    /** Access a vector element. Here we return a reference
    * \param i the first coordinate of the element. By default it's the row
    * \param j the second coordinate of the element. By default it's the column
    * \return a reference to the element
    */
        ...
        mpl::equal< mpl::size_t<M>, mpl::size_t<1> >,
        mpl::equal< mpl::size_t<N>, mpl::size_t<1> > >,
        reference >::type operator () (size_type i) {
            return M == 1 ? at_element (size_type(0), i) :
            at_element (i, size_type(0));
        }
    ...
    // Importing the operators from matrix_expression.
    // This is necessary for proper operation of the tree optimizer.
        using matrix_expression< fixed_matrix<T, M, N, L, A> >::operator=;
        using matrix_expression< fixed_matrix<T, M, N, L, A> >::operator+=;
        using matrix_expression< fixed_matrix<T, M, N, L, A> >::operator-=;
/*
        // Assignment
...
*/
    ...
    /**
        A fixed size row vector of values.
    */
    template <class T, std::size_t M>
    using fixedrowvector = fixed_matrix<T, M, 1>;
    /**
         A fixed size column vector of values.
    */
    template <class T, std::size_t N>
    using fixedcolvector = fixed_matrix<T, 1, N>;
