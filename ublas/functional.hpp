...
namespace boost { namespace numeric { namespace ublas {
    ...
    struct binary_evaluator_traits {
        ...
        typedef typename promote_traits<typename M1::difference_type, typename M2::difference_type>::promote_type difference_type;
        typedef typename promote_traits<typename M1::value_type, typename M2::value_type>::promote_type value_type;
        typedef value_type result_type;
    };
    
    template <typename M1, typename M2>
    struct dmatdmatsum : public binary_evaluator_traits<M1, M2> {
        typedef typename binary_evaluator_traits<M1, M2>::size_type size_type;
        typedef typename binary_evaluator_traits<M1, M2>::value_type value_type;
        typedef typename binary_evaluator_traits<M1, M2>::result_type result_type;
        
        template <typename Xpr1, typename Xpr2>
        static BOOST_UBLAS_INLINE
        result_type apply (const Xpr1& c1, const Xpr2& c2, size_type i, size_type j) {
    ...
    template <typename M1, typename M2>
    struct dmatdmatsub : public binary_evaluator_traits<M1, M2> {
        typedef typename binary_evaluator_traits<M1, M2>::size_type size_type;
        typedef typename binary_evaluator_traits<M1, M2>::value_type value_type;
        typedef typename binary_evaluator_traits<M1, M2>::result_type result_type;
    ...
    template <typename M1, typename M2>
    struct dmatdmatprod : public binary_evaluator_traits<M1, M2> {
        typedef typename binary_evaluator_traits<M1, M2>::size_type size_type;
        typedef typename binary_evaluator_traits<M1, M2>::value_type value_type;
        typedef typename binary_evaluator_traits<M1, M2>::result_type result_type;
        ...
        result_type apply (const Xpr1& c1, const Xpr2& c2, size_type i, size_type j) {
            ...
            if( c1.size2() != 0 ) {
                const size_t end( ( ( c1.size2() - 1 ) & size_t(-2) ) + 1 );
                tmp = c1(i, 0) * c2(0, j);
                for(size_t k = 1; k < end; k += 2) {
                    tmp += c1(i, k) * c2(k, j);
                    tmp += c1(i, k + 1) * c2(k + 1, j);
                }
                if( end < c1.size2() ) {
                    tmp += c1(i, end) * c2(end, j);
                }
            }
