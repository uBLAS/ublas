...
void EstimateFlops(size_t N, lib& name) {
    name.SetFlops( N ); //flop complexity for scale
}

int main(int argc, char **argv){ 
    
    std::string ckernel = "scale";
    ...
    for(size_t NN = N; NN <= Nmax; NN += Ninc){
        ...
        blazemarks.VecScalarRun(ckernel);
    ...
    for(size_t NN = N; NN <= Nmax; NN += Ninc){
        ...
        ublasmarks.VecScalarRun(ckernel);
    ...
    for(size_t NN = N; NN <= Nmax; NN += Ninc){
        ...
        eigenmarks.VecScalarRun(ckernel);
